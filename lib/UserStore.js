...
var Crypto = require('crypto'); //Replace Bcrypt Logic with this, add plugin logic
...
function UserStore(DB, Restrictions, Callback, Options)
{
    ...
    {
        this.DB = DB;
        this.KeyLength = Options && Options.KeyLength ? Options.KeyLength : 512;
        this.Iterations = Options && Options.Iterations ? Options.Iterations : 1000;
...
UserStore.prototype.Add = function(User, Callback) {
    ...
    {
        Callback(new Error('UserStore: NotNull constraint not respected.'));
        return;
    }
...
Add(<User>, <Password>), where <User> is {'Username', 'Email'}. Password is optional, either 'Username' or 'Email' must be specified.
Remove(<User>), where <User> is {'Username', 'Email'}. Either 'Username' or 'Email' must be specified.
Get(<User>, <Password>), where <User> is {'Username', 'Email'}. Either 'Username' or 'Email' must be specified. If 'Password' is specified, it will be checked against the database.
AddMembership(<User>, <Memberships>), where User is {'Username', 'Email'} and 'Membership' is an object whose keys are the groups to add and whole values are the group expiry date (or null if they never expire).
RemoveMembership(<User>, <Memberships>), where User is {'Username', 'Email'} and 'Memberships' is an array of groups (strings) to remove or a string (if removing only 1 group).
ExpandGroupExpiry(<User>,<Group>,<Seconds>).
SetGroupExpiry(<User>,<Group>,<Date>).
SetPassword(<User>, <NewPassword>,<OldPassword>), if <OldPassword> is specified, it will be checked before the operation proceeds.

1) Lazy group expiry evaluation.
2) Just pass <User to query, don't defined it.
*/
